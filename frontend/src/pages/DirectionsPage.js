...
const API_URL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api/directions';
...
export default function DirectionsPage() {
  ...
  const handleEdit = (row) => { setForm(row); setEditId(row.id_direction); setOpenDrawer(true); };
    ...
    <Box p={3}>
      ...
      <Paper sx={{ overflowX: 'auto' }}>
        <table style={{ width: '100%' }}>
          ...
          <tbody>
          {rows.map((e) => (
            <tr key={e.id_direction}>
              <td>{e.id_direction}</td>
              <td>{e.nom}</td>
              <td>{e.description}</td>
              <td>
                <IconButton onClick={() => handleEdit(e)} color="primary" size="small"><EditIcon /></IconButton>
                <IconButton onClick={() => handleDelete(e.id_direction)} color="secondary" size="small"><DeleteIcon /></IconButton>
              </td>
            </tr>
          ))}
          </tbody>
      ...
      <Drawer anchor="right" open={openDrawer} onClose={handleClose} PaperProps={{ sx: { p: 3, width: 340 } }}>
        <Typography variant="h5" mb={2}>{editId ? 'Modifier' : 'Nouveau'} Direction</Typography>
