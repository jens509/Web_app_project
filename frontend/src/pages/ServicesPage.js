...
export default function ServicesPage() {
  ...
  const fetchData = async () => { const { data } = await axios.get(API_URL); setRows(data); };
  useEffect(() => { fetchData(); }, []);
  const handleEdit = (row) => { setForm(row); setEditId(row.id_service); setOpenDrawer(true); };
  const handleNew = () => { setForm(initForm); setEditId(null); setOpenDrawer(true); };
  const handleClose = () => setOpenDrawer(false);
  const handleChange = e => setForm({ ...form, [e.target.name]: e.target.value });
  const handleSubmit = async e => {
    e.preventDefault();
    if (editId) { await axios.put(`${API_URL}/${editId}`, form); }
  ...
  return (
    <Box p={3}>
          ...
          {rows.map((e) => (
            <tr key={e.id_service}>
              ...
              <td>
                ...
                <IconButton onClick={() => handleDelete(e.id_service)} color="secondary" size="small"><DeleteIcon /></IconButton>
              </td>
            </tr>
          ))}
          </tbody>
      ...
      <Drawer anchor="right" open={openDrawer} onClose={handleClose} PaperProps={{ sx: { p: 3, width: 340 } }}>
        <Typography variant="h5" mb={2}>{editId ? 'Modifier' : 'Nouveau'} Service</Typography>
